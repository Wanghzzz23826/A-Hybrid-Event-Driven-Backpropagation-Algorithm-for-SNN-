...
def main():
    ...
    for ep in range(1, epochs + 1):
        ...
        for X, y in dl_tr:
            ...
            if mon["trigger"]:
                ...
                if USE_SPSA and mon["trigger"]:
                    ...
                    mask_rec[torch.tensor(post_idx, device=mask_rec.device)[:, None],
                        torch.tensor(pre_idx,  device=mask_rec.device)[None, :]] = True
                    mask_rec &= model.core.rec_mask.bool()
                    #in
                    mask_in = torch.zeros_like(model.W_in, dtype=torch.bool)
                    mask_in[torch.tensor(post_idx, device=mask_in.device), :] = True
                    ...
                    mon_spsa_rec = spsa_update_Wrec(
                        ...
                        auto_gain=False, target_update_norm=0.25, gain_clip=(0.2, 5.0),
                        mask=mask_rec,
                    )
                    ...
                    mon_spsa_in = spsa_update_Win(
                        ...
                        grad_clip=MainCfg.spsa_grad_clip,
                        clip_norm=MainCfg.spsa_clip_norm,
                        auto_gain=False, target_update_norm=0.15, gain_clip=(0.2, 5.0),
                        mask=mask_in,
                    )
            ...
            history.append({
                "step": global_step , "epoch": ep, "phase": "spsa_only",
        ...
        if len(df_sync):
            ...
            df_ep = hist_df[hist_df["phase"] == "spsa_only"]
            if len(df_ep):
                plt.figure()
                plt.plot(df_ep["step"], df_ep["train_loss"])
                plt.xlabel("Step"); plt.ylabel("Train Loss")
                plt.title("Loss per Step (forward-only; SPSA on triggers)")
                savefig_safe(FIG_DIR / "train_loss_per_step_spsa_only.png")
